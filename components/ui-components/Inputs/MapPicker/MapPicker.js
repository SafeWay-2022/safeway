...
import { MapContainer, Marker, TileLayer, Popup, Circle, useMapEvents } from 'react-leaflet';
...
const MapPicker = ({ list, value, setLimit, refetch }) => {
  ...
  const center =
    !value.length > 0
      ? { lat: list[0]?.geo.lat, lng: list[0]?.geo.lg }
      : { lat: value.lat, lng: value.lg };
  useEffect(() => {
    map && map?.flyTo(center);
  }, [center]);
  return (
    <MapContainer ref={setMap} style={{ height: '900px' }} center={center} zoom={14}>
      <TileLayer
      ...
      {list?.map((elem, i) => {
        return (
          <Marker key={elem?.key || i} position={{ lat: elem.geo.lat, lng: elem.geo.lg }}>
            <Popup>
              <div style={{ display: 'flex' }}>
                <div>
                  <div>{elem?.name} </div>
                  <div>
                    {elem?.country} <span>{elem.city}</span>
                  </div>
