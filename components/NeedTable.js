  ...
  const columns = [
    {
      ...
      dataIndex: 'category',
      render: (category) => {
        return <Tag style={{ fontSize: '14px' }}>{category}</Tag>;
      },
    },
    {
      title: 'Country',
      ...
      render: (coordinates) => {
        return (
          <GeoLocation
            readonly={true}
            withoutInput={false}
            value={coordinates ? coordinates : defaultGeolocationProps}
          />
        );
      },
    },
    ...
    <div className={styles.container}>
      <main>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <div>
            <Radio.Button
              style={!mapView ? { backgroundColor: '#6461BC', color: 'white' } : {}}
        ...
        {!mapView ? (
          ...
          <div style={{ height: '900px' }}>
            <MapPicker value={value} list={list} setLimit={setLimit} />
          </div>
        )}
      </main>
    </div>
